// Package log github.com/snovichkov/zap-gelf
package log
...
func (w *writer) Write(buf []byte) (n int, err error) {
	...
	if n, err = w.conn.Write(cBytes); err != nil {
		fmt.Printf("Failed to send log to Graylog: %v", err)
		return n, err
	}

	if n != len(cBytes) {
		err = fmt.Errorf("wrote %d bytes but should have written %d bytes", n, len(cBytes))
		fmt.Printf("Failed to send log to Graylog: %v", err)
		return n, err
	}
